...
| `PIPEBOARD_API_TOKEN`     | no       | **Defunct as of Aug 2026** — Pipeboard removed its raw-token endpoint (HTTP 410; they no longer hand out the underlying Meta access token), so this fallback can no longer activate. The code detects the 410 and disables itself. Kept for the day a raw-token source returns; safe to unset. |
